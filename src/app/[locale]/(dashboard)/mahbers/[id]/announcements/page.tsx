...
export default function AnnouncementsPage({
  ...
  const readMutation = useMutation({
    mutationFn: (annId: string) =>
      communicationService.markAnnouncementAsRead(id, annId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["mahber-announcements", id] });
    },
  });
              ...
              <Card
                ...
                className={`overflow-hidden transition-all ${
                  announcement.priority === "Urgent"
                    ? "border-l-4 border-l-status-error border-y-border-glass border-r-border-glass"
                    : announcement.priority === "Important"
                      ? "border-l-4 border-l-gold border-y-border-glass border-r-border-glass"
                      : ""
